"""
train_ct_cv.py  --  CT-only baseline on the PET cohort, 5-fold patient-level CV.
...
Outputs (per seed) under figures/petct/ct_only_s<seed>/:
...
from ct_cv_dataset import CTCohortDataset, CLASS_NAMES, CLASS_SHORT, SUBTYPE_TO_LABEL
...
def train_one_fold(tr_pat, va_pat, seed, epochs, patience, fixed_epochs=False):
    ...
    tr = CTCohortDataset(inner_tr, is_train=True)
    iva = CTCohortDataset(inner_va, is_train=False)
    va = CTCohortDataset(va_pat, is_train=False)          # outer fold, untouched
...
def main():
    ...
    OUT = f"figures/petct/ct_only_{tag}_s{args.seed}"
...
def _report(df, OUT):
    ...
    for tag, (yt, pd_, prob2) in {
        ...
        _confmat(yt, pd_, f"CT-only {tag}-level", os.path.join(OUT, f"cm_{tag}.png"))
        for idx, mname in [(0, "precision"), (1, "recall"), (2, "f1")]:
            _bars([pr, rc, f1][idx], mname, f"CT-only {tag}: {mname} by class",
...
def _pr_curve(y, score, path):
    ...
    ax.set_title("CT-only crop-level: one-vs-rest PR", fontsize=11)
