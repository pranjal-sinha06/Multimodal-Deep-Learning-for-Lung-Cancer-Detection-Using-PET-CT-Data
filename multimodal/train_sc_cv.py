...
def main():
    ...
    OUT = f"figures/petct/sc_{tag}_s{args.seed}"
...
def _report(df, OUT):
    ...
    for tag, (yt, pd_, prob2) in {
        ...
        _confmat(yt, pd_, f"Secondary Capture {tag}-level", os.path.join(OUT, f"cm_{tag}.png"))
        for idx, mname in [(0, "precision"), (1, "recall"), (2, "f1")]:
            _bars([pr, rc, f1][idx], mname, f"Secondary Capture {tag}: {mname} by class",
...
def _pr_curve(y, score, path):
    ...
    ax.set_title("Secondary Capture crop-level: one-vs-rest PR", fontsize=11)
