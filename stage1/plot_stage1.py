...
for tag in sorted(found):
    ...
    ax2 = ax1.twinx()
    ax2.plot(df.epoch, df.val_map_50, marker="o", color="#2E75B6", label="val mAP@0.5")
    ax2.plot(df.epoch, df.val_map, marker="s", ls="--", color="#4C72B0", label="val mAP@0.5:0.95")
    ax2.set_ylabel("validation mAP", color="#2E75B6")
    ax2.tick_params(axis="y", labelcolor="#2E75B6")
    ax2.set_ylim(0, 1)
    ax2.axvline(df.epoch[bi], ls=":", color="grey", alpha=0.7)

    lines = ax1.get_lines() + ax2.get_lines()
    ax1.legend(lines, [l.get_label() for l in lines], fontsize=8, loc="center right")
    plt.title(f"Stage 1 {tag} \u2014 training loss vs validation mAP\n"
              f"(different metrics: compare shape, not value)", fontsize=10)
    plt.tight_layout()
